...
void LegacyCameraProviderImpl_2_4::addDeviceNames(int camera_id, CameraDeviceStatus status, bool cam_new)
{
    ...
    ALOGE("%s: " "camera_id = %d", __func__, camera_id);
    ...
    if (deviceVersion >= CAMERA_DEVICE_API_VERSION_3_2 &&
        ...
        ALOGE("%s: isOpenLegacyDefined", __func__);
        ...
        if (ret == 0) {
            ...
            mCameraDeviceNames.add(deviceNamePair);
            ALOGE("%s: mCameraDeviceNames.add %s", __func__, cameraIdStr.c_str());
            if (cam_new) {
                mCallbacks->cameraDeviceStatusChange(deviceNamePair.second, status);
            }
...
bool LegacyCameraProviderImpl_2_4::initialize() {
    ...
    for (int i = 0; i < mNumberOfLegacyCameras; i++) {
        ...
        //ALOGE("%s: Camera info query start!! mNumberOfLegacyCameras = %d,"
        //        "camera_id = %d",
        //        __func__, mNumberOfLegacyCameras, i);
            //ALOGE("%s: Camera info query inside!! mNumberOfLegacyCameras = %d,"
        //        "camera_id = %d",
        //        __func__, mNumberOfLegacyCameras, i);
            //ALOGE("%s: Camera info query inside!! i = %d", __func__, i);
            auto rc = mModule->getCameraInfo(i, &info);
            if (rc != NO_ERROR) {
                ALOGE("%s: Camera info query failed!", __func__);
                //ALOGE("%s: info rc %d", __func__, rc);
                //mModule.clear();
                //return true;
            } else {
    ...
    struct camera_info info1;
    struct camera_info info2;
    struct camera_info info3;
    struct camera_info info4;
    mModule->getCameraInfo(2, &info1);
    mModule->getCameraInfo(3, &info2);
    mModule->getCameraInfo(4, &info3);
    mModule->getCameraInfo(5, &info4);
...
int LegacyCameraProviderImpl_2_4::checkCameraVersion(int id, camera_info info) {
    ...
    //ALOGE("%s: " "camera_id = %d", __func__, id);
...
Return<void> LegacyCameraProviderImpl_2_4::getCameraDeviceInterface_V3_x(
    ...
    std::string cameraId, deviceVersion;
        ALOGE("%s: " "camera_id = %s deviceHame = %s", __func__, cameraId.c_str(), cameraDeviceName.c_str());
    bool match = matchDeviceName(cameraDeviceName, &deviceVersion, &cameraId);
    if (!match) {
        _hidl_cb(Status::ILLEGAL_ARGUMENT, nullptr);
        return Void();
